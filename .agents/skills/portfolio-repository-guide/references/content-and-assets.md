...
- Los períodos viven una sola vez en `cv/content.json.timelines.companies`: `kind: calendar` usa intervalos `start`/`end` en `YYYY-MM` (`end: null` significa vigente); `kind: duration` usa `months` sin inventar fechas. Cada experiencia/proyecto referencia `timeline_id`; una fase puede referenciar `timeline_period` o usar `context` cuando necesita aclarar el alcance sin inventar una fecha. El generador deriva de esos datos las etiquetas de los hitos. `timelines.as_of` fija el mes límite usado al validar los períodos. El mantenimiento posterior se describe aparte.
- La presentación elegida es una línea de tiempo vertical: círculos negros alineados con los títulos y conectados por una línea gris, con fecha o contexto y viñetas al lado. `VerticalTimeline` mide los párrafos para ubicar cada hito; el espacio vertical depende del contenido, no de la duración. En experiencia profesional los cargos comparten una línea y House of CB incluye dos hitos internos para separar productos. En freelance cada empresa tiene su propio bloque y nodo principal; sus etapas usan una sangría de 14 puntos y ramas desde la línea de esa empresa. La línea termina en la última etapa y no conecta con la siguiente empresa.
